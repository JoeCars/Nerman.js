...
describe("Nouns tests", () => {
	test("should construct with string", () => {
		const nouns = new Nouns(process.env.ALCHEMY_URL!, { shouldIgnoreCacheInit: true });
	...
	test("should change polling time with option", () => {
		const nouns1 = new Nouns(process.env.ALCHEMY_URL!, { shouldIgnoreCacheInit: true, pollingTime: 1 });
		const nouns2 = new Nouns(process.env.ALCHEMY_URL!, { shouldIgnoreCacheInit: true });
