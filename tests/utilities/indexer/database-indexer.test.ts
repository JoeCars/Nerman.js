import { indexEvent, Indexer } from "../../../src/utilities/indexer/database-indexer";
...
describe("formatDate tests", () => {
	it("should work when passing in a date object", () => {
		...
		const result = Indexer.formatDate(date);
	...
	it("should work when passing in a timestamp", () => {
		...
		const date = new Date("2023-12-25");
		const result = Indexer.formatDate(date.getTime() / MILLISECONDS_PER_SECOND);
		expect(result).toEqual("2023-12-25");
	});
});
...
describe("incrementDate tests", () => {
	it("should increment days", () => {
		const result = Indexer.incrementDate("2023-12-25");
		expect(result).toEqual("2023-12-26");
	});
	it("should increment months", () => {
		const result = Indexer.incrementDate("2023-01-31");
		expect(result).toEqual("2023-02-01");
	});
	it("should increment years", () => {
		const result = Indexer.incrementDate("2023-12-31");
		expect(result).toEqual("2024-01-01");
	});
	it("should handle leap years", () => {
		const result1 = Indexer.incrementDate("2024-02-28");
		const result2 = Indexer.incrementDate("2024-02-29");
		const result3 = Indexer.incrementDate("2023-02-28");
